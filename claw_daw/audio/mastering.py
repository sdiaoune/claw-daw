...
MASTER_PRESETS: dict[str, MasterPreset] = {
    ...
    "clean": MasterPreset(
        ...
        afilter="highpass=f=30,lowpass=f=18000,loudnorm=I=-14:TP=-2.0:LRA=11",
    ),
    ...
    "demo": MasterPreset(
        ...
        afilter="highpass=f=30,lowpass=f=17500,acompressor=threshold=-20dB:ratio=3:attack=3:release=40,equalizer=f=9000:t=h:width_type=o:width=2:g=2,alimiter=limit=0.98",
    ),
    ...
    "punchy": MasterPreset(
        name="punchy",
        afilter="highpass=f=30,lowpass=f=17500,acompressor=threshold=-24dB:ratio=4:attack=3:release=80,asoftclip=type=tanh,alimiter=limit=0.97",
    ),
}
...
def master_wav(
    ...
    *,
    ...
    filters = [base]
    auto_fade_in = float(fade_in_seconds)
    auto_fade_out = float(fade_out_seconds)
    if trim_seconds is not None:
        if auto_fade_in <= 0.0:
            auto_fade_in = 0.003
        if auto_fade_out <= 0.0:
            auto_fade_out = 0.01
    fade = _fade_filter(fade_in_seconds=auto_fade_in, fade_out_seconds=auto_fade_out, dur_seconds=trim_seconds)
    if fade:
        filters.append(fade)
